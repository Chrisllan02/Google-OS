...
export default function App() {
  ...
  // --- OMNI-BAR LOGIC (OPERATIONAL AI) ---
  const handleCommandIntent = (text: string): boolean => {
      const lower = text.toLowerCase();
      
      // App Launching Intents
      if (lower.includes('abrir drive') || lower.includes('ir para o drive')) {
          openApp('drive');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Abrindo o Google Drive...' }]);
          return true;
      }
      if (lower.includes('criar documento') || lower.includes('novo doc')) {
          openApp('doc');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Criando um novo Documento Google...' }]);
          return true;
      }
      if (lower.includes('criar planilha') || lower.includes('nova planilha')) {
          openApp('sheet');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Criando uma nova Planilha...' }]);
          return true;
      }
      if (lower.includes('criar slide') || lower.includes('nova apresentação')) {
          openApp('slide');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Criando nova Apresentação...' }]);
          return true;
      }
      if (lower.includes('ver email') || lower.includes('abrir gmail') || lower.includes('checar email')) {
          openApp('mail');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Abrindo sua Caixa de Entrada...' }]);
          return true;
      }
      if (lower.includes('minha agenda') || lower.includes('calendário')) {
          openApp('calendar');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Abrindo Google Agenda...' }]);
          return true;
      }
      if (lower.includes('nova tarefa') || lower.includes('criar tarefa')) {
          openApp('tasks');
          setChatHistory(prev => [...prev, { role: 'assistant', text: 'Abrindo Tarefas...' }]);
          return true;
      }
  ...
  const handleSendMessage = async () => {
    ...
    // 1. Check for Operational Intent
