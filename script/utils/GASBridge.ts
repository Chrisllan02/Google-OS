...
class GASBridge {
  ...
  async toggleTask(id: string | number, listId?: string): Promise<boolean> { if (this.isGasEnvironment()) return new Promise((resolve) => { (window as any).google.script.run.withSuccessHandler((res: string) => resolve(JSON.parse(res).success)).toggleTask(id, listId); }); return Promise.resolve(true); }
  ...
  async deleteTask(id: string | number, listId?: string): Promise<boolean> { if (this.isGasEnvironment()) return new Promise((resolve) => { (window as any).google.script.run.withSuccessHandler((res: string) => resolve(JSON.parse(res).success)).deleteTask(id, listId); }); return Promise.resolve(true); }
