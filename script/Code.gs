...
function getInitialData() {
  ...
  // O Dashboard usa apenas dados resumidos, mantemos leve
  var emails = getEmailsMockOrReal();
  var events = getEventsMockOrReal();
  var files = getRecentFilesSimple(); // Versão simplificada para o widget do dashboard
...
// --- FUNÇÕES ESPECÍFICAS DO DRIVE APP ---

function getDriveItems(params) {
  var folderId = params.folderId;
  var category = params.category || 'root'; 
  var query = params.query || ''; // Nova funcionalidade de busca
...
/**
 * Upload de arquivo via Base64 string
 */
...
/**
 * Obtém conteúdo do arquivo (Base64) para Preview
 */
  ...
  try {
    ...
    return threads.map(function(t) {
      ...
      return {
        ...
        preview: m.getPlainBody().substring(0,60)+"...",
        read: !t.isUnread()
      };
    });
  } catch (e) { return []; }
...
function sendEmail(to, subject, body) {
  try { GmailApp.sendEmail(to, subject, body); return true; } catch(e) { return false; }
}
