        ...
        <div className="flex-1 flex overflow-hidden relative p-3 gap-3">
            ...
            <div className="flex-1 flex flex-col bg-[#1E1E1E] rounded-[24px] border border-white/5 overflow-hidden relative">
                <div className="flex flex-col border-b border-white/5 bg-[#1E1E1E] shrink-0">
                    ...
                    <div className="flex items-center gap-2 px-4 pt-3 pb-1">
                         ...
                         {showNewMenu && (
                            <div className="absolute top-[60px] right-4 w-48 bg-[#2d2e30] border border-white/10 rounded-xl shadow-2xl py-1 z-50">
                                <button onClick={() => { setActivePane('compose'); setShowNewMenu(false); }} className="w-full text-left flex items-center gap-3 px-4 py-3 hover:bg-white/5"><Mail size={16}/><span className="text-sm">Novo E-mail</span></button>
                    ...
                    {/* Advanced Filter Panel (Floating) */}
                    <AdvancedFilterPanel 
            ...
            <div className={`flex flex-col bg-[#1E1E1E] rounded-[24px] border border-white/5 overflow-hidden relative transition-all duration-0 ease-linear`} style={{ width: rightPanelWidth }}>
                ...
                <div className={`absolute inset-0 top-14 bottom-0 w-full bg-[#1E1E1E] transition-opacity duration-300 ${activePane === 'settings' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
                    {/* Settings Content... Same as previous */}
                </div>
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className="p-4 border-t border-white/5 bg-[#1E1E1E] shrink-0">
                                <div className="flex gap-3">
                                    <button className="flex-1 border border-white/10 rounded-full py-3 px-4 text-left text-sm text-white/40 hover:bg-white/5 transition-colors flex items-center gap-2"><Reply size={16}/> Responder</button>
                                    <button className="flex-1 border border-white/10 rounded-full py-3 px-4 text-left text-sm text-white/40 hover:bg-white/5 transition-colors flex items-center gap-2"><Forward size={16}/> Encaminhar</button>
                                </div>
                            </div>
                        </div>
                    ) : (
                ...
                {/* EVENT CREATE/EDIT PANE */}
                <div className={`absolute inset-0 top-14 bottom-0 w-full bg-[#1E1E1E] transition-opacity duration-300 ${activePane === 'event-create' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
                    <div className="flex flex-col h-full p-6 text-white overflow-y-auto">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-medium">{activeEvent?.id ? 'Editar Evento' : 'Novo Evento'}</h2>
                            <button onClick={() => setActivePane('agenda')} className="p-2 hover:bg-white/10 rounded-full text-white/70"><X size={18}/></button>
                        </div>
                        <div className="space-y-6">
                            <input type="text" placeholder="Adicionar título" className="w-full bg-transparent text-2xl font-light outline-none border-b border-white/10 pb-2 focus:border-blue-500 transition-colors" value={activeEvent?.title || ''} onChange={(e) => setActiveEvent({...activeEvent, title: e.target.value})} autoFocus />
                            <div className="flex gap-4">
                                <div className="flex-1 space-y-1">
                                    <label className="text-xs text-white/50 uppercase">Início</label>
                                    <input type="datetime-local" className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white outline-none focus:border-blue-500" value={activeEvent?.start ? new Date(activeEvent.start).toISOString().slice(0, 16) : ''} onChange={(e) => setActiveEvent({...activeEvent, start: new Date(e.target.value)})} />
                                </div>
                                <div className="flex-1 space-y-1">
                                    <label className="text-xs text-white/50 uppercase">Fim</label>
                                    <input type="datetime-local" className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white outline-none focus:border-blue-500" value={activeEvent?.end ? new Date(activeEvent.end).toISOString().slice(0, 16) : ''} onChange={(e) => setActiveEvent({...activeEvent, end: new Date(e.target.value)})} />
                                </div>
                            </div>
                            <div className="pt-4 border-t border-white/10 flex justify-end">
                                <button onClick={handleSaveEvent} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-full font-medium text-sm transition-colors">Salvar</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {/* EVENT VIEW PANE */}
                <div className={`absolute inset-0 top-14 bottom-0 w-full bg-[#1E1E1E] transition-opacity duration-300 ${activePane === 'event-view' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
                    {activeEvent && (
                        <div className="flex flex-col h-full p-6 text-white">
                            ...
                            <div className="space-y-6 flex-1 overflow-y-auto">
                                <div className="flex items-start gap-4">
                                    <div className={`w-4 h-4 rounded mt-1.5 ${activeEvent.color || 'bg-blue-500'}`}></div>
                                    <div>
                                        <h2 className="text-2xl font-light mb-1">{activeEvent.title}</h2>
                                        <p className="text-sm text-white/70">{new Date(activeEvent.start).toLocaleString()} – {new Date(activeEvent.end).toLocaleTimeString()}</p>
                                    </div>
                                </div>
                                {activeEvent.location && (
                                    <div className="flex items-center gap-4">
                                        <MapPin size={20} className="text-white/50"/>
                                        <span className="text-sm text-white/80">{activeEvent.location}</span>
                                    </div>
                                )}
                                {activeEvent.description && (
                                    <div className="flex items-start gap-4">
                                        <AlignLeft size={20} className="text-white/50 mt-0.5"/>
                                        <p className="text-sm text-white/80 leading-relaxed whitespace-pre-wrap">{activeEvent.description}</p>
                                    </div>
                                )}
                                <div className="flex items-start gap-4">
                                    <Users size={20} className="text-white/50 mt-0.5"/>
                                    <div className="flex-1">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-sm text-white/80">Convidados</span>
                                            <span className="text-xs text-white/40 bg-white/5 px-2 py-0.5 rounded-full">{activeEvent.guests?.length || 0}</span>
                                        </div>
                                        <div className="space-y-2">
                                            {activeEvent.guests?.map((g:any, i:number) => (
                                                <div key={i} className="flex items-center gap-2 p-2 hover:bg-white/5 rounded-lg transition-colors">
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white ${g.color || 'bg-gray-600'}`}>{g.avatar || g.name[0]}</div>
                                                    <span className="text-xs text-white/70">{g.name}</span>
                                                </div>
                                            ))}
                                            <div className="flex items-center gap-2 mt-2">
                                                <Plus size={16} className="text-white/40"/>
                                                <input type="text" placeholder="Adicionar convidados" className="bg-transparent text-xs text-white outline-none flex-1 py-1" value={guestInput} onChange={(e) => setGuestInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddGuest()} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
