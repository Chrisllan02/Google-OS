...
import { Lightbulb, Square, Settings, X, Search, Plus, Image as ImageIcon, Paintbrush, MoreVertical, Archive, Trash2, Pin } from 'lucide-react';
            ...
            {sortedNotes.length === 0 ? (
                ...
                <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
                    {sortedNotes.map((n:any) => {
                        ...
                        return (
                            <div key={n.id} className={`rounded-xl p-4 shadow-sm break-inside-avoid cursor-default group transition-all border hover:border-white/40 relative ${noteColorClass}`}>
                                <button 
                                    onClick={() => togglePin(n)}
                                    className={`absolute top-2 right-2 p-1.5 rounded-full hover:bg-black/20 transition-opacity ${n.pinned ? 'opacity-100 text-white' : 'opacity-0 group-hover:opacity-100 text-white/50'}`}
                                >
