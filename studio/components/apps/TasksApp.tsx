...
import { CheckCircle2, Settings, X, Search, Star, Plus, Trash2, Calendar, GripVertical } from 'lucide-react';
...
export default function TasksApp({ onClose, data }: TasksAppProps) {
  ...
  const handleAddTask = async () => {
      ...
      setNewTaskTitle('');
      // Mantém o input focado para adicionar múltiplas tarefas
      inputRef.current?.focus();
      
      // Backend call
      await bridge.createTask(newTaskTitle);
  };
            ...
            <div className="max-w-3xl mx-auto bg-[#1E1E1E] rounded-2xl overflow-hidden shadow-2xl border border-white/10 flex flex-col h-full md:h-auto md:min-h-[500px]">
                ...
                <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5 shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="bg-blue-600 text-xs px-2 py-0.5 rounded-full">{tasks.filter(t => !t.completed).length}</span>
                    </div>
                    <button 
                ...
                <div className="divide-y divide-white/5 flex-1 overflow-y-auto custom-scrollbar">
                    {filteredTasks.length === 0 ? (
                        ...
                        filteredTasks.map((t:any) => (
                            <div key={t.id} className="p-4 flex items-start gap-4 hover:bg-white/5 transition-all cursor-default group animate-in fade-in duration-300">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {!t.completed && <div className="flex gap-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <span className="text-[10px] border border-white/10 rounded-full px-2 py-0.5 text-white/50 flex items-center gap-1"><Calendar size={10}/> Hoje</span>
                                    </div>}
                                </div>
