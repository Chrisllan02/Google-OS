...
const TaskNode: React.FC<TaskNodeProps> = ({ task, depth = 0, activeTask, setActiveTask, toggleTask }) => (
    <div className="flex flex-col">
        <div 
            onClick={() => setActiveTask(task)}
            className={`p-4 flex items-start gap-4 hover:bg-white/5 transition-all cursor-pointer group animate-in fade-in duration-300 ${task.completed ? 'opacity-50' : 'opacity-100'} ${activeTask?.id === task.id ? 'bg-blue-500/10 border-l-4 border-blue-500' : 'border-l-4 border-transparent'}`}
        ...
        >
            ...
            <button 
                ...
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all duration-300 shrink-0 ${task.completed ? 'bg-blue-500 border-blue-500 scale-110' : 'border-white/40 hover:border-white'}`}
            >
                {task.completed && <CheckCircle2 size={14} className="text-white"/>}
            </button>
            <div className="flex-1 min-w-0">
                <p className={`text-sm transition-all duration-300 ${task.completed ? 'text-white/40 line-through' : 'text-white'}`}>{task.title}</p>
                {task.details && <p className="text-xs text-white/40 mt-1 truncate flex items-center gap-1"><Mail size={10} className="text-blue-400"/> {task.details}</p>}
                <div className="flex gap-2 mt-1">
                    {task.date && <span className="text-[10px] border border-white/10 rounded-full px-2 py-0.5 text-white/50 flex items-center gap-1 w-fit"><Calendar size={10}/> {new Date(task.date).toLocaleDateString()}</span>}
                </div>
            </div>
        </div>
        {/* Note: This component in Studio version doesn't seem to support subtasks in data structure yet, 
            but if it did, recursive rendering would go here similar to main app */}
    </div>
);
...
export default function TasksApp({ onClose, data, onUpdate }: TasksAppProps) {
  ...
  const toggleTask = async (id: string | number) => {
      ...
      await bridge.toggleTask(id.toString());
  };
  ...
  const deleteTask = async (id: string | number) => {
      ...
      await bridge.deleteTask(id.toString());
  };
                    ...
                    <div className="divide-y divide-white/5 flex-1 overflow-y-auto custom-scrollbar">
                        {filteredTasks.length === 0 ? (
                            ...
                            filteredTasks.map((t:any) => (
                                <TaskNode key={t.id} task={t} activeTask={activeTask} setActiveTask={setActiveTask} toggleTask={toggleTask} />
                            ))
                        )}
                    </div>
                </div>
